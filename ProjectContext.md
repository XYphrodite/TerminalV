...
Проверки быстрого запуска: 168 Node/браузерных, 24 PTY/транспортных (включая установленную Ubuntu: ввод, UTF-8, Linux home без shell-профилей) и 11 SQLite — всего 203, без пропусков при заданных тестовых PowerShell 7 и Ubuntu. Release-сборка успешна. Неблокирующие сообщения сборки: npm allow-scripts для esbuild и Vite о JS-бандле чуть больше 500 kB. Превью меню: `artifacts/launch-menu-preview.png`. WPF/WebView2 с рабочим host не запускался; пакет/обновление новых исходников ещё не проверены, коммит и релиз не сделаны.
